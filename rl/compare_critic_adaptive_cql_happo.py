"""Compare critic-adaptive CQL with legacy CQL and HAPPO."""
...
def main() -> None:
    ...
    adaptive=CriticAdaptiveCQL(
        state_features, seed=args.seed, normalize_rewards=False
    )
    adaptive.load_legacy_values(args.legacy_cql)
    started=time.perf_counter(); training=adaptive.train(dataset,args.updates,batch_size=args.batch_size)
    training_seconds=time.perf_counter()-started
    adaptive.save(args.output/"critic_adaptive_cql.pt")
    happo=LegacyHAPPO(args.happo)
    results={
        "legacy_cql":evaluate(legacy,"legacy_cql",args),
    ...
    report={
        ...
        "critic_adaptive_training":{"seconds":training_seconds,"parameters":adaptive.parameter_count,**training},
        "results":results,
        "collapse_diagnosis":{"healthy":"Q(data) above Q(OOD), non-zero Q spread, alpha not pinned at maximum.","over_conservative":"Q(OOD)-Q(data) far below target, alpha near minimum, flat Q spread.","under_conservative":"Q(OOD)-Q(data) above target, alpha rising toward maximum."},
        "scope":"All policies use identical example-scenario episodes and seeds. No online sampling occurs during CQL training.",
    }
    (args.output/"critic_adaptive_report.json").write_text(json.dumps(report,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
